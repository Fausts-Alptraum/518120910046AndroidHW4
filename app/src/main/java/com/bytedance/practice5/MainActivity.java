...
public class MainActivity extends AppCompatActivity {
    ...
    public List<Message> baseGetMessagesFromRemote(String studentId) {
        if (studentId == null){
            studentId = "";}
        String urlStr = String.format("%smessages?student_id=%s", BASE_URL,studentId);
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (!result.success) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(MainActivity.this, "请求不成功" , Toast.LENGTH_SHORT).show();
                        }
                    });
                } else{
                }
